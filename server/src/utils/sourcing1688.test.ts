...
describe("estimate1688", () => {
  const s = normalise1688Settings(DEFAULT_1688_SETTINGS);
...
describe("settings", () => {
  it("start off, with the 5-years-on-1688 supplier filter", () => {
  ...
  it("build one actor run for all enabled keywords", () => {
    ...
    expect(actorInput(s)).toMatchObject({ mode: "search", keywords: ["a", "c"], maxItems: 20, supplierYears: "5", minOrderQuantity: 3, includeSkuDetails: false });
  });
