      ...
      <div className="flex-1 overflow-y-auto p-5">
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-amber-200 p-6 text-center max-w-lg mx-auto mt-10">
            ...
            <button onClick={() => setTab("products")} className="mt-4 px-4 py-2 rounded-lg text-white text-sm font-semibold" style={{ background: "#B8862E" }}>Add your first product</button>
          </div>
        ) : null}
...
function SupplierImport({ sellerId, onImported }: { sellerId: string; onImported: () => void }) {
  ...
  const importItem = async (item: R) => {
    ...
    const retailPrice = draft ? Number(draft) : undefined;
  ...
  return (
    <div>
      <div className="mb-4">
        ...
        <p className="text-xs text-gray-500">Pick an item from Ballylife's vetted China / Japan / South Korea supplier catalog, see the supplier's price, and set your own retail price (your profit margin) before it becomes a listing in your store. Sourcing, warehouse QC, customs, and delivery are all handled centrally — you never deal with the supplier directly. Once live, price changes need marketplace-team approval.</p>
      </div>
    ...
    <div className="bg-white rounded-xl border border-gray-100 p-5 max-w-md">
      ...
      <label className="block mb-3">
        ...
        <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3}
          className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-[#B8862E]" />
      </label>
